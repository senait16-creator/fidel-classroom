// =============================================================================
// JS/TEAM/LEVELS.JS
// The embedded level map (renderEmbeddedLevelMap) was part of the retired
// Team Hub screen and is no longer mounted anywhere — it's kept only so
// nothing crashes if it's ever called. The family practice sheet
// (openEmbeddedFamilyPractice / familyPracticeSheet) is still very much
// alive: it's the modal the Letter Board (js/letterboard.js) opens when a
// student taps a letter family outside the Level flow.
//
...
let embeddedPracticeReturnTo = null; // 'letterBoard' | null — where "← Back" should go
...
function closeEmbeddedFamilyPractice() {
    ...
    // Refresh level map to show updated progress (no-op now that the
    // Team Hub screen that used to mount these is retired)
    ...
    activeChallengeContext = {
        ...
        onStreakPassed: async (finalStreak) => {
            ...
            setTimeout(() => {
                ...
                showPostStreakWritingPrompt(fidelObj, levelNumber, 'embedded');
            }, 1800);
        }
    };
...
// returnMode is passed explicitly by the caller ('embedded' or 'challenge')
// instead of being guessed from the embeddedActiveFamily global — that
// global is set from three different entry points (Letter Board, the
// embedded level map, the Today card) and nothing guarantees it gets
// cleared before a student switches over to the Fidel Challenge flow, so
// guessing from it could land a student on the wrong screen — or an
// unpopulated one, which reads as the page going blank — after passing a
// streak there.
function showPostStreakWritingPrompt(fidelObj, levelNumber, returnMode) {
...
// This prompt is shown after any matching-game streak passes, whichever
// flow launched the game — the Letter Board's practice sheet or the
// Competition family detail screen. returnMode is passed explicitly by
// showPostStreakWritingPrompt (see comment there) rather than guessed from
// embeddedActiveFamily, which can be stale.
function dismissPostStreakPrompt(returnMode) {
    document.getElementById('postStreakPrompt')?.remove();
    const mode = returnMode || (embeddedActiveFamily ? 'embedded' : 'challenge');

    if (mode === 'embedded') {
        document.getElementById('familyPracticeSheet').style.display = "flex";
        if (embeddedActiveFamily) renderWritingStatusForFamily(embeddedActiveFamily.base, 'practiceSheetWritingStatusBox');
        return;
    }

    if (typeof activeChallengeFamilyObj !== "undefined" && activeChallengeFamilyObj) {
        document.getElementById('challengeFamilyDetailScreen').style.display = "block";
        renderWritingStatusForFamily(activeChallengeFamilyObj.base);
        if (typeof refreshChallengeDetailWritingGate === "function") {
            refreshChallengeDetailWritingGate(activeChallengeFamilyObj, activeChallengeFamilyLevel);
        }
    }
}

function submitFromPostStreak(baseLetter, levelNumber, returnMode) {
    document.getElementById('postStreakPrompt')?.remove();
    const mode = returnMode || (embeddedActiveFamily ? 'embedded' : 'challenge');
