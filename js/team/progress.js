        ...
        if (encourageBtn) {
            encourageBtn.onclick = () => {
                if (typeof shareTeamChallenge === 'function') {
                    // Plain text for the native share sheet / clipboard — icon()
                    // returns SVG markup, which would paste as raw code instead
                    // of a symbol, so a real emoji character goes here instead.
